...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for _ in range(n):
        if sample is None:
            ...
            population, weights = zip(*sample_model.items())
        ...
        for page in pagerank:
            ...
            for possible in corpus:
                if page in corpus[possible]:
                    p += pagerank[possible] / len(corpus[possible])
                if not corpus[possible]:
                    p += pagerank[possible] / len(corpus)
            newrank[page] = (1 - damping_factor) / len(corpus) + damping_factor * p
        repeat = False
        for page in pagerank:
